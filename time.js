...
const KEY="projectTimePlannerV2";
let state={selected:{},qty:{},hours:{}};
...
const fmt=n=>n<1?Math.round(n*60)+"m":n.toFixed(n<10?1:0)+"h";
const save=()=>localStorage.setItem(KEY,JSON.stringify(state));
function total(){
 let raw=0,count=0;
 GROUPS.flatMap(g=>g.items).forEach(i=>{let id=i[0];if(!state.selected[id])return;count++;raw+=(+state.qty[id]||0)*(+state.hours[id]||0)});
 return {raw,count};
}
function calc(){
 const t=total(); const totalHours=t.raw; const days=totalHours/7.5;
 $("selectedCount").textContent=t.count;
 $("totalTime").textContent=fmt(totalHours);
 $("workingDays").textContent=days.toFixed(1);
 $("finalTime").textContent=totalHours.toFixed(1)+" hours";
 $("finalDays").textContent="Approximately "+days.toFixed(1)+" working days at 7.5 hours/day";
 $("timeRange").textContent=fmt(totalHours*.9)+"–"+fmt(totalHours*1.15);
 document.querySelectorAll("[data-calc]").forEach(el=>{const id=el.dataset.calc;el.textContent=state.selected[id]?fmt(state.qty[id]*state.hours[id]):"0h"});
}
function render(){
 $("taskGroups").innerHTML=GROUPS.map(g=>`<section class="group">
   ...
   <div class="task-table">
    <div class="task-row head"><div>Activity</div><div>Qty</div><div>Time / item</div><div>Estimated time</div></div>
    ${g.items.map(i=>{const [id,name,desc]=i,active=state.selected[id];return `<div class="task-row ${active?"":"off"}">
      ...
      <div class="calc" data-calc="${id}">0h</div>
    </div>`}).join("")}
   </div></section>`).join("");
...
$("copyBtn").onclick=()=>{
 const t=total(); const days=t.raw/7.5; const list=GROUPS.flatMap(g=>g.items).filter(i=>state.selected[i[0]]).map(i=>"• "+i[1]).join("\n");
 const text=`Project Time Estimate\n\nEstimated time: ${t.raw.toFixed(1)} hours\nApprox. working days: ${days.toFixed(1)}\nSuggested range: ${fmt(t.raw*.9)}–${fmt(t.raw*1.15)}\n\nIncluded activities:\n${list}`;
